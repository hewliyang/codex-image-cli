...
These prompt recipes are for the `img-gen` CLI.
...
The labeled lines are prompt scaffolding, not a closed schema. `Asset type` and `Input images` are prompt-only scaffolding; pass local inputs with repeated `img-gen --input <path>` flags and describe their roles in the prompt.

Execution details such as `--size`, `--quality`, `--background`, `--input`, and `--output` are `img-gen` flags. For transparent images, try `img-gen --background transparent` first; if the output is not clean, prompt for a flat chroma-key background and remove it with the bundled `scripts/remove_chroma_key.py` helper.

CLI notes:
- `img-gen` writes PNG output.
- `--quality` supports `low`, `medium`, `high`, and `auto`.
- For 4K-style output, use `3840x2160` or `2160x3840`.
- Image inputs are provided with repeated `--input <path>` flags; label each input role in the prompt.
...
Post-process note: after chroma-key generation, run `python <skill-dir>/scripts/remove_chroma_key.py --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`. Validate the alpha channel, transparent corners, subject coverage, and edge fringing before shipping.
