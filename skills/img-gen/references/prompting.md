...
These prompting principles are for the `img-gen` CLI.

This file is about prompt structure, specificity, and iteration. CLI execution controls such as `--quality`, `--size`, `--background`, `--input`, and `--output` are covered in `../SKILL.md`.
...
- Use `--quality medium` or `--quality high` for small text, dense infographics, data-heavy slides, multi-font layouts, legends, axes, and footnotes.
...
## Transparent images
- Try `img-gen --background transparent` first for transparent-image requests. If the result is not clean, use the chroma-key workflow.
...
- After chroma-key generation, remove the background locally with `python <skill-dir>/scripts/remove_chroma_key.py --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill` and validate the alpha result before shipping it.
...
- Use `--quality low` for fast drafts and thumbnails; use `--quality medium`, `--quality high`, or `--quality auto` for final assets, dense labels, diagrams, and identity-sensitive edits.
- Use `--size` for canvas control. For 4K-style output, use `3840x2160` for landscape or `2160x3840` for portrait.
- Use repeated `--input <path>` flags for edit targets and references, and label each image role in the prompt.
- Use `--output <path>` for project-bound assets. Do not leave project assets only in the default output directory.
- Use `--background transparent` for native transparent attempts; use chroma-key plus `scripts/remove_chroma_key.py` if needed.
...
- infographic-diagram: Define the audience and layout flow; label parts explicitly; require verbatim text; prefer `--quality high` for dense labels.
...
- background-extraction: For simple opaque subjects, request a clean cutout on a perfectly flat chroma-key background; crisp silhouette; generous padding; no shadows; no halos; preserve label text exactly; no restyling. For complex subjects, expect to iterate and validate alpha edges carefully.
